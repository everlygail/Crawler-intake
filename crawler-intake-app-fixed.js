...
function detectContradiction(currentAnswer,currentTags){
  // A contradiction follow-up is clarification, not new evidence.
  // Never allow a follow-up to generate another follow-up.
  if(state.current?.isContradiction || state.answers.length<2)return null;
...
function shouldFinish(){
  ...
  // Once a clarification has been answered, it is allowed to complete
  // the interview like any other response.
...
function chooseNext(){
  ...
  const unusedFinal=QUESTION_BANK.filter(q=>q.stage==="final"&&!used.has(q.id));
  const unusedAdaptive=QUESTION_BANK.filter(q=>q.stage==="adaptive"&&!used.has(q.id));
  const hasAnsweredFinal=state.answers.some(a=>String(a.id).startsWith("final_"));

  // Move toward a final calibration once enough evidence exists, but never
  // assume an unused final question still exists.
  if(state.answers.length>=MAX_QUESTIONS-1){
    return pick(unusedFinal)||pick(unusedAdaptive)||null;
  }

  if(state.answers.length>=MIN_QUESTIONS && !hasAnsweredFinal &&
     (state.confidence>=70 || state.answers.length>=10)){
    return pick(unusedFinal)||pick(unusedAdaptive)||null;
  }

  const ranked=Object.entries(state.scores).sort((a,b)=>b[1]-a[1]).map(x=>x[0]);
  const focus=ranked.slice(0,3);
  let pool=unusedAdaptive.filter(q=>q.tags.some(t=>focus.includes(t)));
  if(!pool.length)pool=unusedAdaptive;

  // Absolute fallback order guarantees this function never returns an
  // accidental undefined value from pick([]).
  return pick(pool)||pick(unusedFinal)||null;
}
...
function renderQuiz(){
  ...
  // Last-resort watchdog: an interview can never remain on a blank or
...
async function submitAnswer(answer,btn){
  btn.disabled=true;
  const answeringFollowUp=Boolean(state.current?.isContradiction);
  const tags=analyze(answer);
  const contradiction=answeringFollowUp?null:detectContradiction(answer,tags);
  ...
  if(contradiction && state.followUpsAsked<3){
    ...
    const nextQuestion=state.queue.length?state.queue.shift():chooseNext();

    // Fail closed: if every eligible question has already been consumed,
    // finish the dossier rather than rendering an undefined question.
    if(!nextQuestion){
      state.step="processing";
      render();
      return;
